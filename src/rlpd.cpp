...
entry_t::entry_t(const std::vector<int>& new_tag_indices, const std::vector<char>& new_image_data) {
...
void PRNFile::write(const std::string& filepath) {
  ...
  std::cout << "wrote seperator: " << "1" << std::endl;

  for (int i = 0; i < (int)entries.size(); i++) {
    std::cout << "begin entry" << std::endl;
    
    for (int j = 0; j < (int)entries[i].tag_indices.size(); j++) {
      file.write(reinterpret_cast<const char*>(&entries[i].tag_indices[j]), sizeof(int));

      std::cout << "wrote tag index: " << entries[i].tag_indices[j] << std::endl;
    }
    ...
    file.write(reinterpret_cast<const char*>(&entries[i].image_end_off), sizeof(int));

    std::cout << "wrote image end offset: " << entries[i].image_end_off << std::endl;
...
void PRNFile::read(const std::string& filepath) {
  ...
  while (!file.eof()) {
    while (file.peek() != 1) {
      ...
      new_entry.tag_indices.push_back(tag_index); // ur such a nerd but i admire it
    }

    file.read(reinterpret_cast<char*>(&new_entry.image_end_off), sizeof(int));
...
void PRNFile::add_tag(const std::string& tag) {
  tag_pool.push_back(tag);
}

void PRNFile::add_entry(const entry_t& entry) {
